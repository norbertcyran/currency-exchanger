...
describe("My First Test", () => {
  ...
  it('register',()=>{
    ...
    cy.get('input').eq(0).type("adamparke1")
    cy.get('input').eq(1).type("adamparke1@gmail.com")
    cy.get('input').eq(2).type("Boston1991")
    cy.get('input').eq(3).type("Boston1991")
    cy.get('input').eq(4).type("adam")
    cy.get('input').eq(5).type("Park")
    cy.get('input').eq(6).type("567374845")
    cy.get('input').eq(7).type("Przylaszczkowa 5f{enter}")
    cy.wait(400)
  ...
  it("money transfer",()=>{
    ...
    cy.get('input').eq(0).type("macikoj3")
